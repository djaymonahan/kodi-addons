...
ADDON = xbmcaddon.Addon( id = 'script.torrent-tv.rpi' )
ADDON_ICON	 = ADDON.getAddonInfo('icon')
ADDON_PATH = ADDON.getAddonInfo('path')
ADDON_ICON	 = ADDON.getAddonInfo('icon')
DATA_PATH = xbmc.translatePath( os.path.join( "special://profile/addon_data", 'script.torrent-tv.rpi') )
VERSION = '1.5.3'
...
def showMessage(message = '', heading='Torrent-TV.RPI', times = 6789):
    try: 
...
def GET(target, post=None, cookie = None, tryies = 0):
    try:
        ...
        req.add_header('User-Agent', 'XBMC (script.torrent-tv.rpi)')
        if cookie:
