...
class WMainForm(xbmcgui.WindowXML):
    ...
    def getChannels(self, param):
        if not self.category_tmp:
            ...
            if param == 'channel':
                ...
                self.showStatus("Ожидайте: cоздание списка каналов...")
                defines.showMessage("Создание списка каналов...", 'Ожидайте:', 15000)
            ...
            for ch in jdata['channels']:
                ...
                chname = ch["name"]
                #LogToXBMC('Add channel: %s' % chname.encode('utf-8'))
                if ch["access_user"] == 0:
                    chname = "[COLOR FF99D8FF]%s[/COLOR]" % chname
            ...
            if param == 'channel':
                defines.showMessage('Список каналов создан', 'Torrent-TV.AML', 2000)
    ...
    def fillChannels(self):
        self.showStatus("Заполнение списка каналов")
        if not self.list:
    ...
    def fillTranslation(self):
        if not self.list:
            self.showStatus("Список не инициализирован")
            return
        self.showStatus("Заполнение списка трансляций")
        self.list.reset()
